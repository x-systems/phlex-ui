...
class Demo extends \Phlex\Ui\Columns
{
    public $left;
    public $right;
    ...
    protected function extractCodeFromClosure(\Closure $fx): string
    {
        ...
        if ($funcRefl->getEndLine() === $funcRefl->getStartLine()) {
            throw new \Phlex\Ui\Exception('Closure body to extract must be on separate lines');
        }
    ...
    public function setCodeAndCall(\Closure $fx, $lang = 'php')
    {
        ...
        $this->highLightCode();
        \Phlex\Ui\View::addTo(\Phlex\Ui\View::addTo($this->left, ['element' => 'pre']), ['element' => 'code'])->addClass($lang)->set($code);
    ...
    public function highLightCode()
    {
        if (!self::$isInitialized) {
            ...
            $this->js(true, (new \Phlex\Ui\JsChain('hljs'))->initHighlighting());
