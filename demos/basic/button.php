...
Header::addTo($webpage, ['Basic Button', 'size' => 2]);
...
Header::addTo($webpage, ['Properties', 'size' => 2]);
...
Header::addTo($webpage, ['Custom Template', 'size' => 2]);

$view = View::addTo($webpage, ['template' => new HtmlTemplate('Hello, {$tag1}, my name is {$tag2}')]);

Button::addTo($view, ['World'], ['tag1']);
Button::addTo($view, ['Phlex UI', 'blue'], ['tag2']);

Header::addTo($webpage, ['Attaching', 'size' => 2]);

Button::addTo($webpage, ['Previous', 'top attached']);
Table::addTo($webpage, ['attached', 'header' => false])
