<?php

declare(strict_types=1);

namespace Phlex\Ui\Tests;

use Phlex\Ui\Table;

class GridTest extends \Phlex\Core\PHPUnit\TestCase
{
    use Concerns\HandlesTable;

    public $m;

    protected function setUp(): void
    {
        $a = [
            1 => ['id' => 1, 'email' => 'test@test.com', 'password' => 'abc123', 'xtra' => 'xtra'],
            2 => ['id' => 2, 'email' => 'test@yahoo.com', 'password' => 'secret'],
        ];
        $this->m = new MyModel(new \Phlex\Data\Persistence\Array_($a));
    }

    public function test1(): void
    {
        $t = new Table();
        $t->initialize();
        $t->setModel($this->m, false);

        $t->addColumn('email');
        $t->addColumn(null, new Table\Column\Template('password={$password}'));

        $this->assertSame('<td>{$email}</td><td>password={$password}</td>', $t->getDataRowHtml());
        $this->assertSame(
            '<tr data-id="1"><td>test@test.com</td><td>password=abc123</td></tr>',
            $this->extractTableRow($t)
        );
    }

    public function test1a(): void
    {
        $t = new Table();
        $t->initialize();
        $t->setModel($this->m, false);

        $t->addColumn('email');
        $t->addColumn('password');

        $this->assertSame('<td>{$email}</td><td>***</td>', $t->getDataRowHtml());
        $this->assertSame(
            '<tr data-id="1"><td>test@test.com</td><td>***</td></tr>',
            $this->extractTableRow($t)
        );
    }

    public function test2(): void
    {
        $t = new Table();
        $t->initialize();
        $t->setModel($this->m, ['email']);
        $del = $t->addColumn(null, [Table\Column\Delete::class]);

        $this->assertSame('<td>{$email}</td><td><a href="#" title="Delete {$email}?" class="' . $del->elementId . '"><i class="ui red trash icon"></i>Delete</a></td>', $t->getDataRowHtml());
        $this->assertSame(
            '<tr data-id="1"><td>test@test.com</td><td><a href="#" title="Delete test@test.com?" class="' . $del->elementId . '"><i class="ui red trash icon"></i>Delete</a></td></tr>',
            $this->extractTableRow($t)
        );
    }

    public function test3(): void
    {
        $t = new Table();
        $t->initialize();
        $t->setModel($this->m, ['email']);
        $t->addColumn('xtra', null, ['type' => 'password']);

        $this->assertSame('<td>{$email}</td><td>***</td>', $t->getDataRowHtml());
        $this->assertSame(
            '<tr data-id="1"><td>test@test.com</td><td>***</td></tr>',
            $this->extractTableRow($t)
        );
    }
}

class MyModel extends \Phlex\Data\Model
{
    public $titleKey = 'email';

    protected function doInitialize(): void
    {
        parent::doInitialize();

        $this->addField('email');
        $this->addField('password', ['type' => 'password']);
    }
}
